...
public class AltspaceBuildWindow : EditorWindow
{
    ...
    private void UploadToAltspaceVR()
    {
        ...
        EditorUtility.DisplayProgressBar("Uploading template", "(Please wait, progress update not yet implemented)", 0.5f);
        string envPath = Application.dataPath + Path.DirectorySeparatorChar + "Plugins";
        ...
        string errorStrings = "";
        while (!processInfo.StandardOutput.EndOfStream)
        {
            string output = processInfo.StandardOutput.ReadLine();
            if (string.IsNullOrEmpty(output))
            {
                continue;
            }
            else if (output.ToLower().Contains("error"))
            {
                errorsFound = true;
                errorStrings += output + " ";
        ...
        if (errorsFound == true)
        {
            bool option = EditorUtility.DisplayDialog("Error during upload",
                $"The following error was returned from backend: {errorStrings}", "OK");
